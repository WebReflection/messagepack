...
try {
  ...
  // eval needed due Firefox ...
  // eval(`data = (await import('./carts.json', { with: { type: 'json' } })).default`);
...
const jspack = new JSPack({ circular, minimumByteLength: 0xFFFF });
...
const bench = async (name, encode, decode) => {
  ...
  console.timeEnd('total');
  console.timeEnd('combined total');
...
runAll && await bench('jspack', jspack.encode, jspack.decode);
